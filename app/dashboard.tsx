...
type Device = {
  ...
  processes?: Array<{ pid: number; npu_process_name?: string; user?: string; container?: { name?: string } }>;
};
...
function NpuDie({ device, detail = false }: { device: Device; detail?: boolean }) {
  const hbm = percent(device.hbm?.used_mb, device.hbm?.total_mb);
  const active = clampPercent(device.aicore_percent) > 0;
  return <span className={`npu-die ${detail ? 'npu-die--detail' : ''} ${hbm != null && hbm >= 32 ? 'is-dark' : ''}`} style={{ backgroundColor: dieColor(hbm) }} title={`NPU ${device.npu_id} · HBM ${number(hbm, '%')} · AICore ${number(device.aicore_percent, '%')}`}><b>{device.npu_id}</b>{detail && <small>HBM {number(hbm, '%')}</small>}{active && <i className="npu-die__activity" aria-label="AICore 活跃" />}</span>;
}
...
export default function Dashboard() {
  ...
  const effectiveServerId = overview.servers.some((server) => server.id === selectedServerId) ? selectedServerId : (overview.servers[0]?.id || '');
  const selectedServer = overview.servers.find((server) => server.id === effectiveServerId) || overview.servers[0];
  ...
  const filteredServers = overview.servers.filter((server) => `${server.name} ${server.host} ${server.tags.join(' ')}`.toLowerCase().includes(serverSearch.toLowerCase()));
