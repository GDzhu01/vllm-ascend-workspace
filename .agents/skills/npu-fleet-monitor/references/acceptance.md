...
- the platform service reports `ActiveState=active`, `SubState=running`, and `UnitFileState=enabled` (`systemd --user` on Linux, `windows-hkcu-run` on Windows);
...
On Windows, `status` reports the supervisor PID and log path. Inspect that log when health fails; the per-user `HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run` entry starts the hidden supervisor at sign-in without administrator privileges.
