...
- 启动时自动发现 Git 公共目录对应主工作区的 `.vaws-local/machine-inventory.json`，导入尚未纳管的宿主机，并通过已有 SSH 密钥补装监控专用公钥。
...
- 固定服务器侧栏汇总在线状态、NPU 数量、HBM 和 CPU；总览以 NPU die 展示逐卡 HBM 占用，0%–50% 由浅蓝渐变、50% 以上固定深蓝，AICore 非零时以右上角红色闪光点提示。点击服务器可查看逐卡 AICore/HBM、温度、功耗、进程、磁盘与 Docker 明细。
